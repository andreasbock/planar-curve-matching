...
"""
Convergence test for WuXu element (example 1 from the paper).
"""
n = 2  # two dimensions
m = n + 1
...
def solve_ex1(h):
    ...
    uex = exp(pi*y)*sin(pi*x) # solution from paper
    uex = interp(uex, V)

    uh = Function(V)

    a = inner(grad(delta(u)), grad(delta(v)))*dx
    L = - inner(grad(delta(uex)), grad(delta(v)))*dx
...
for h in res:
    ...
    l2, h1, h2, h3, exec_time = solve_ex1(h)
    l2s.append(l2)
    h1s.append(h1)
    h2s.append(h2)
    h3s.append(h3)
...
path = "../../tex/"
with open(path + 'wuxu1.csv', mode='w+') as cfile:
