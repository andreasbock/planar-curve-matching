...
"""
Convergence test for WuXu element (example 2 from the paper).
"""
n = 2  # two dimensions
m = n + 1
...
def solve_ex2(k):
    mdir = "l-shaped_meshes/"
    mesh = Mesh(mdir + "l-shaped_{}.msh".format(k))
    ...
    V_ = FunctionSpace(mesh, "CG", 1)
    u, v = TrialFunction(V), TestFunction(V)

    # solution from paper
    uex = (x**2 + y**2)**1.25 * sin(2.5*atan_2(x, y)) # atan_2(y,x)? but fails!
    uex_fn = project(uex, V)

    a = inner(grad(delta(u)), grad(delta(v)))*dx
    L = - inner(grad(delta(uex_fn)), grad(delta(v)))*dx
    ...
    return sqrt(assemble((uh)**2*dx)), \
           sqrt(assemble(inner(grad(uh), grad(uh))*dx)), \
           sqrt(assemble(inner(div(grad(uh)), div(grad(uh)))*dx)), \
           sqrt(assemble(inner(grad(div(grad(uh))), grad(div(grad(uh))))*dx)), \
           exec_time
...
for k in range(6):
    PETSc.Sys.Print("k = {}".format(k))
    l2, h1, h2, h3, exec_time = solve_ex2(k)
    l2s.append(l2)
    h1s.append(h1)
    h2s.append(h2)
    h3s.append(h3)
...
path = "../../tex/"
with open(path + 'wuxu2.csv', mode='w+') as cfile:
